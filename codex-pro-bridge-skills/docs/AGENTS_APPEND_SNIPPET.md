...
Before the first GPT Pro browser round, verify that the Codex Chrome extension is installed and enabled. In the extension's **Details**, enable **Allow access to file URLs** or local bundles may not attach.
...
Use ChatGPT's visible attachment control. Verify the visible filename and exact requested model with `check_browser_preflight.py`, submit once, and do not retry while generation remains active. UI selection does not attest backend identity.

Capture model/upload/timing provenance, then run `verify_bridge_thread.py --require-complete-rounds` before continuing or handing off.
